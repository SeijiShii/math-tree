...
export default async function handler(req: any, res: any) {
  ...
  const stepIndex = Number.isFinite(Number(body?.stepIndex)) ? Number(body.stepIndex) : 0
  const r = await gradeAnswer(db(), { slug, stepIndex, latex }, makeAiClientFromEnv())
  if (!r.found) {
    res.status(404).json({ error: 'Not Found' })
    return
  }
  res.status(200).json({ match: r.match, viaAi: r.viaAi, hint: r.hint ?? null })
}
