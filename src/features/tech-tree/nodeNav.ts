...
/** そのノードが学習に進めるか（未解放ノードの導線は無効、SPEC §6.1）。 */
export function canLearn(state: ProgressState): boolean {
  return state !== 'locked'
}

/** ノード選択時の遷移先パス。進めない / slug 無しなら null（no-op）。 */
export function learnTarget(node: { slug?: string; canLearn?: boolean }): string | null {
