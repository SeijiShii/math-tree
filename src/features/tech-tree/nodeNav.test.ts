...
import { canLearn, learnTarget, miniMapNodeColor, MINIMAP_MASK_COLOR } from './nodeNav'
...
describe('tech-tree nodeNav（C20260622-001 ノード選択→学習遷移）', () => {
  ...
  it('R1/R2: 解放済みノードは /learn/<slug> を返す', () => {
    expect(learnTarget({ slug: 'seisuu', canLearn: true })).toBe('/learn/seisuu')
    expect(learnTarget({ slug: 'mojishiki', canLearn: true })).toBe('/learn/mojishiki')
  })
